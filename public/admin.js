...
// ── Realtime refresh ──────────────────────────────────────────────────────────
const adminRealtime = { timer: null, running: false, last: {} };

function activeAdminTab() {
  return document.querySelector('.tab-btn.active')?.dataset.tab || 'stats';
}

function shouldAdminPoll(key, everyMs, force=false) {
  const now = Date.now();
  if (force || !everyMs) {
    adminRealtime.last[key] = now;
    return true;
  }
  if ((adminRealtime.last[key] || 0) + everyMs > now) return false;
  adminRealtime.last[key] = now;
  return true;
}

async function refreshAdminRealtime(force=false) {
  if (document.hidden && !force) return;
  if (adminRealtime.running) return;
  adminRealtime.running = true;
  try {
    const tab = activeAdminTab();
    if ((tab === 'stats' || force) && shouldAdminPoll('stats', REALTIME?.ADMIN_STATS_MS, force)) await loadStats();
    if (tab === 'users' && shouldAdminPoll('users', REALTIME?.ADMIN_LIST_MS, force)) await adminLoadUsers();
    if (tab === 'kyc' && shouldAdminPoll('kyc', REALTIME?.ADMIN_LIST_MS, force)) await adminLoadKyc();
    if (tab === 'withdrawals' && shouldAdminPoll('withdrawals', REALTIME?.ADMIN_LIST_MS, force)) await adminLoadWithdrawals();
    if (tab === 'rates' && shouldAdminPoll('rates', REALTIME?.ADMIN_LIST_MS, force)) await loadCurrentRates();
    if (tab === 'ledger' && shouldAdminPoll('ledger', REALTIME?.ADMIN_LEDGER_MS, force)) await adminLoadLedger();
  } finally {
    adminRealtime.running = false;
  }
}

function startAdminRealtime() {
  if (adminRealtime.timer) clearInterval(adminRealtime.timer);
  adminRealtime.timer = setInterval(() => { refreshAdminRealtime(false); }, REALTIME?.TICK_MS || 5000);
